    ...
    public function store(Request $request)
    {
        ...
        if ($user->tokenCan("1")) {
            ...
            if ($request->hasFile('image')) {
                ...
                $fileName = time() . '.' . $image->getClientOriginalExtension(); //ตั้งชื่อไฟล์ตาม timestemp ที่อัปโหลดรูปภาพ
                $uploadFolder = public_path('uploads'); //อ่านโฟลเดอร์ public

                // ตรวจสอบว่าโฟลเดอร์ uploads มีหรือไม่ อันนี้คือกำหนด permission ให้เลย
                if (!File::exists($uploadFolder)) {
                    File::makeDirectory($uploadFolder, 0755, true);
                }

                $image->move($uploadFolder, $fileName); //เอาไฟล์เข้าไปในโฟลเดอร์
